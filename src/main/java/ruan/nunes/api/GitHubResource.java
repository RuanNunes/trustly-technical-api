...
public class GitHubResource implements GitHubApi {
	...
	public RepositoryDTO getInfo(final String url) {
		final List<GitHubHtml> gitHubHtmlsHome = gitHubHtmlService.getGitHubHtmls(Rest.GET(url));
        List<File> files = fileService.getFiles(gitHubHtmlsHome);
		return new RepositoryDTO(url,files);
	}
}
